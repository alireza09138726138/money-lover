...
$result = mysqli_query($conn, "select * from produ where bank='".$bank."' and Account='".$Account."'  AND user_id = '".$login_id."' ORDER BY id DESC LIMIT 0 , 1") or die(mysqli_error ($result));
...
if (isset($_POST['submite1']))
 {
	 ...
	 $cost=$_POST['cost'];
     $Limite=$_POST['Limite'];
	 $sql = "SELECT * FROM `produ` WHERE bank='".$bank."' AND Account='".$Account."'  AND user_id = '".$login_id."' ORDER BY id DESC LIMIT 0 , 1;";   
	 $query =  mysqli_query($conn, $sql);
	 $data = mysqli_fetch_array($query);
	 $cost1 =$data['cost'];
	 $cost2 =$cost1 + $cost;
$sql ="INSERT INTO `produ` (`id`, `cost`, `bank`, `Account`, `Limite`, `user_id`, `username`) VALUES (NULL,'$cost2', '$bank','$Account' ,'$Limite','$login_id','$login_session')";				
